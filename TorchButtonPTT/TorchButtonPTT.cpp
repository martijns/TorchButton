...
int _tmain(int argc, _TCHAR* argv[])
{
    ...
    // Replace exe
    std::wstring find(L"TorchButtonPTT.exe");
    ...
    {
        MessageBox(NULL, L"Error finding TorchButtonPTT.exe in current filename!", L"Error", MB_OK | MB_ICONERROR);
        return 1;
    }

    // Create string with switch
    std::wstring exefilearg(exefile);
    exefilearg.append(L" /ptt");
    ...
    if (!bWorked)
    {
        MessageBox(NULL, L"Error starting the TorchButton.exe process with /ptt parameter!", L"Error", MB_OK | MB_ICONERROR);
        return 1;
    }
